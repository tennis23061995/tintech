...
var _class = function (_think$model$base) {
    ...
    _class.prototype.getArticleList = function getArticleList(where) {
        return this.model("article").where(where).order("createtime DESC").limit(3).select();
    };
    //返回点击排行列表
    ...
    _class.prototype.getPageVideoSelect = function getPageVideoSelect(where, page, pagesize) {
        return this.model("video").where(where).order("createtime DESC").page(page, pagesize).select();
    };
    //分页thinkjs分页数据


    _class.prototype.getPageVideoCountSelect = function getPageVideoCountSelect(where, page, pagesize) {
        return this.model("video").where(where).order("createtime DESC").page(page, pagesize).countSelect();
    };

    _class.prototype.getReplyListInfo = function getReplyListInfo(where) {
        return this.model('article_comment').field("*,li_article_comment.id as tid,li_article_comment.createtime as mytime").where(where).join({
            user: { on: "author,name" }
        }).select();
    };

    _class.prototype.increpoint = function increpoint(where, count) {
