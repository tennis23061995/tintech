...
$(function () {
    ...
    $("#previewArticle").on('click', function () {
        ...
        if (pid === "") {
            dosubmit('/admin/content/doadd', function (json) {
                if (json.errno === 0) {
            ...
            //更新文章状态
            dosubmit('/admin/video/doadd/', function (json) {
                if (json.errno === 0) {
    ...
    //公用方法
    var dosubmit = function (httpUrl, callback) {
        if (!$('#form-article-add').valid()) {
           alert("co loi")
            return;
        }
        var ispublished = $("#ispublished").is(':checked') ? 1 : 0;
