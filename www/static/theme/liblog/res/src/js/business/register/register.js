    ...
    $("#register").click(function(){
      // $.ajax({
      //     // 获取id，challenge，success（是否启用failback）
      //     url: "/home/register/checkcaptcha", // 加随机数防止缓存
      //     data:{  captchasecret:grecaptcha.getResponse(),__CSRF__:G_csrf},
      //     type: "post",
      //     dataType: "json",
      //
      // }).done(function(data){
            ...
            if(form){
              ...
              var newData={
                ...
                password:$("#password").val(),
                  email:$("#email").val(),
                  captchasecret:grecaptcha.getResponse(),
                __CSRF__:$("#csrf").val()
              }
              //提交请求
                ...
                $.ajax({
                    ...
                    success:function(json){
                      ...
                      console.log($("."+json.tags).parents(".register-item"))
                          showError($("."+json.tags),json.errmsg);
                      }
                    }
                })
                  ...
                  success:function(json){
                      if(json.status!==0){
                        ...
                        _this.after('<span class="isright" id="basic-addon1">√</span>');
                          ...
                          showError(_this,'Tài khoản đã tồn tại!');
      ...
      var _this=$(this);
          ...
          if((_this.val()).length>0){
            ...
            if (!(/^\w+@([0-9a-zA-Z]+[.])+[a-z]{2,4}$/.test(_this.val()))) {
              showError(_this,'Email không hợp lệ!');
  						return false
  					}else{
              $.ajax({
                  ...
                  success:function(json){
                    if(json.status!==0){
                      ...
                      showError(_this,'Email đã tồn tại!');
                      _this.parents('.register-item').addClass('has-error')
                      _this.after('<span class="iswrong" id="basic-addon1">x</span>');
                    }
                  }
