...
RoxyUtils.GetFileIcon = function (a) {
    ret = "/static/filemanager/images/filetypes/unknown.png";
    if (fileTypeIcons[RoxyUtils.GetFileExt(a).toLowerCase()]) {
        ret = "/static/filemanager/images/filetypes/" + fileTypeIcons[RoxyUtils.GetFileExt(a).toLowerCase()]
    }
    return ret
};
...
RoxyUtils.LoadLang = function () {
    ...
    if (RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == "auto") {
        if (RoxyUtils.GetUrlParam("langCode")) {
            a = RoxyUtils.GetUrlParam("langCode").substr(0, 2).toLowerCase();
        }
    }
    $.ajax({
        url: "/admin/fileman/getlang",
        dataType: "json",
        data: {
            lang:a,
            __CSRF__: G_csrf
...
function Directory(a, c, b) {
    ...
    this.GetHtml = function () {
        ...
        d += '<div><img src="/static/filemanager/images/' + (this.dirs > 0 ? "dir-plus.png" : "blank.gif") + '" class="dirPlus" width="9" height="9">';
        d += '<img src="/static/filemanager/images/folder.png" class="dir"><span class="name">' + this.name + " (" + this.files + ")</span></div>";
        d += "</li>";
        return d
    };
    ...
    this.Select = function (e) {
        ...
        d.children("img.dir").prop("src", "/static/filemanager/images/folder.png");
        this.SetStatusBar();
    ...
    this.SetOpened = function () {
        ...
        if (d.find("li").length < 1) {
            d.children("div").children(".dirPlus").prop("src", "/static/filemanager/images/blank.gif")
        } else {
            if (this.IsExpanded()) {
                d.children("div").children(".dirPlus").prop("src", "/static/filemanager/images/dir-minus.png")
            } else {
                d.children("div").children(".dirPlus").prop("src", "/static/filemanager/images/dir-plus.png")
            }
        }
    };
        ...
        $.ajax({
            ...
            success: function (h) {
                ...
                for (i = 0; i < h.data.length; i++) {
                    var j = new Directory(h.data[i].p, h.data[i].d, h.data[i].f);
                    j.Show()
                }
                $("#pnlLoadingDirs").hide();
                $("#pnlDirList").show();
            ...
            $.ajax({
                ...
                success: function (j) {
                    for (i = 0; i < j.data.length; i++) {
                        e.push(new File(j.data[i].p, j.data[i].s, j.data[i].t, j.data[i].w, j.data[i].h))
                    }
                    h.FilesLoaded(e, f)
                },
                error: function (j) {
...
function dragFileOver() {
    $(this).children("img.dir").attr("src", "/static/filemanager/images/folder-green.png")
}

function dragFileOut() {
    $("#pnlDirList").find("img.dir").attr("src", "/static/filemanager/images/folder.png")
}
...
function makeDragDir(b) {
    ...
    return '<div class="pnlDragDir" data-path="' + a.fullPath + '"><img src="/static/filemanager/images/folder.png" align="absmiddle">&nbsp;' + a.name + "</div>"
}
...
function previewFile() {
    var a = getSelectedFile();
    if (a) {
        window.open("/static/upload/"+a.fullPath)
    }
}
...
//issuse https://stackoverflow.com/questions/31214724/gm-conversion-issue-in-node-js/31224811
...
function switchView(a) {
    ...
    if (a == "thumb") {
        $("#pnlFileList .icon").attr("src", "/static/filemanager/images/blank.gif");
...
function setFile() {
    ...
    var a ="/static/upload/"+c.fullPath;
    if (RoxyFilemanConf.RETURN_URL_PREFIX) {
